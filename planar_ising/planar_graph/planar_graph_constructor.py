...
from .planar_graph import planar_graph_nb_type, PlanarGraph
...
def construct_subgraph(graph, subgraph_vertices_mask, subgraph_edges_mask):
    ...
    new_vertices_mapping = common_utils.repeat_int(-1, graph.size)
    ...
    edges_count = 0
    new_edge_indices_mapping = common_utils.repeat_int(-1, graph.edges_count)
    ...
    incident_edge_example_indices = common_utils.repeat_int(-1, len(vertex_costs))
...
def clone_graph(graph):
    ...
    _, _, graph = construct_subgraph(graph, common_utils.repeat_bool(True, graph.size),
...
def _construct_from_ordered_adjacencies(ordered_adjacencies):
    ...
    vertex_costs = common_utils.repeat_float(1/vertices_count, vertices_count)

    edges, edge_indices_by_adjacencies = \
            _create_edges_and_map_adjacencies(ordered_adjacencies)

    incident_edge_example_indices = common_utils.repeat_int(-1, vertices_count)
