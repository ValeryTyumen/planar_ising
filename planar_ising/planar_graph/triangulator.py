...
class Triangulator:
    ...
    def _add_edges_to_connect_graph_components(graph):
        ...
        component_example_vertices = -np.ones(components_count, dtype=int)
        ...
        for component_index in range(components_count - 1):
            ...
            for vertex in [vertex1, vertex2]:
                ...
                if vertex_edge_index == -1:
                    ...
                    vertex_next_edge_index = graph.edges.get_next_edge_index(vertex_edge_index,
                            vertex)
    ...
    @staticmethod
    def _insert_edge(graph, triangle_vertex1, triangle_vertex2, triangle_vertex3,
            triangle_edge_index1, triangle_edge_index2):
        ...
        for vertex in range(graph.size):
            for edge_index in graph.get_incident_edge_indices(vertex):
                ...
                while face_vertices[0] != face_vertices[3] or face_vertices[1] != face_vertices[4]:
                    ...
                    Triangulator._insert_edge(graph, face_vertices[0], face_vertices[1],
                            face_vertices[2], face_edge_indices[0], face_edge_indices[1])

                    face_vertices, face_edge_indices = \
                            Triangulator._get_consequtive_face_vertices_and_edge_indices(graph,
                            face_vertices[2], face_edge_indices[2], 5)
