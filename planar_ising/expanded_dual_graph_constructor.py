...
from lipton_tarjan import PlanarGraphEdges, planar_graph_edges_nb_type, PlanarGraph, \
        planar_graph_nb_type
...
def construct(graph):
    ...
    expanded_dual_graph_incident_edge_example_indices = utils.repeat_int(-1,
            expanded_dual_graph_vertices_count)

    first_dual_edges_mapping = utils.repeat_int(-1, dual_edges_count + intercity_edges_count)
    second_dual_edges_mapping = utils.repeat_int(-1, dual_edges_count + intercity_edges_count)
...
_set_odd_orientation = \
        utils.make_traverse_graph_via_post_order_dfs(_set_parent_edge_odd_orientation,
        Tuple((int32[:], int32[:])))
...
def _get_odd_orientation(graph):
    ...
    edges_mask = utils.repeat_bool(True, graph.edges_count)
...
def get_kasteleyn_orientation(graph, graph_edges_mapping, expanded_dual_graph):
    ...
    kasteleyn_orientation = utils.repeat_int(-1, expanded_dual_graph.edges_count)
...
def get_expanded_dual_subgraph_perfect_matching(graph, graph_edges_mapping, spin_values):
    ...
    perfect_matching_mask = utils.repeat_bool(False, graph_edges_mapping.size)
