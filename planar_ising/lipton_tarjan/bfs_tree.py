...
from ..planar_graph import PlanarGraphEdges, PlanarGraph, PlanarGraphConstructor, search_utils
...
class BFSTree:
    ...
    def construct_bfs_levels(root, graph):
        ...
        search_utils.traverse_graph_via_bfs(root, graph, used_vertex_flags, levels,
                BFSTree._set_adjacent_vertex_level)
    ...
    def construct_bfs_tree_edges_mask(root, graph):
        ...
        search_utils.traverse_graph_via_bfs(root, graph, used_vertex_flags, tree_edges_mask,
                BFSTree._add_edge_to_tree)
    ...
    def collapse_bfs_subtree(graph, bfs_levels, subtree_max_level, bfs_tree_edges_mask):
        ...
        ordered_bfs_subtree_adjacencies, ordered_bfs_subtree_incidence_indices = \
                BFSTree._get_ordered_bfs_subtree_adjacencies_and_incidence_indices(graph,
                bfs_levels, subtree_max_level, bfs_tree_edges_mask)

        bfs_subtree_mask = (bfs_levels <= subtree_max_level)

        new_vertices_mapping, new_edge_indices_mapping, new_graph = \
                PlanarGraphConstructor.construct_subgraph(graph, ~bfs_subtree_mask,
        ...
        for index, (edge_index, adjacent_vertex) in enumerate(zip(
                ordered_bfs_subtree_incidence_indices, ordered_bfs_subtree_adjacencies)):
        ...
        new_new_graph_incident_edge_example_indices = \
                ...
                np.array([root_incident_edge_example_index])))
    ...
    def _get_ordered_bfs_subtree_adjacencies_and_incidence_indices(graph, bfs_levels,
        ...
        if not np.any(bfs_subtree_edges_mask):
            ...
            for edge_index in search_utils.iterate_subgraph_incidence_indices(graph,
                    bfs_subtree_edges_mask, common_utils.repeat_bool(True, graph.edges_count),
                    start_vertex_on_subtree, start_edge_index_on_subtree):
    ...
    def record_bfs_tree_parent_edge_indices_and_total_descendants_costs(graph, bfs_tree_root,
        ...
        parent_edge_indices = search_utils.traverse_graph_via_post_order_dfs(bfs_tree_root, graph,
                bfs_tree_edges_mask, total_descendants_costs,
                BFSTree._update_parent_total_descendants_costs)
