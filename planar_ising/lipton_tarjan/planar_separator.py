...
from .tree_cycles import TreeCycles
from . import separation_class
from .. import common_utils
from ..planar_graph import PlanarGraph, PlanarGraphConstructor, Triangulator, search_utils
...
class PlanarSeparator:
    ...
    def mark_separation(graph):
        ...
        if max_component_cost <= 1/3:
            return PlanarSeparator._mark_separation_when_components_less_than_one_third(
                    connected_component_indices, connected_component_costs)

        if max_component_cost <= 2/3:
            return PlanarSeparator._mark_separation_when_components_less_than_two_thirds(
                    connected_component_indices, max_component_index)

        separation = PlanarSeparator._mark_separation_for_one_connected_component(graph,
                connected_component_indices, max_component_index)

        separation = PlanarSeparator._swap_separation_parts_if_cost_of_first_part_is_smaller(
                graph.vertex_costs, separation)

        separation = PlanarSeparator._fill_undefined_with_second_part(separation)
    ...
    def _mark_separation_for_one_connected_component(graph, connected_component_indices,
        ...
        bfs_levels = BFSTree.construct_bfs_levels(bfs_tree_root, graph)
        ...
        level_one = PlanarSeparator._find_level_one(bfs_level_costs)
        ...
        level_zero = PlanarSeparator._find_level_zero(bfs_level_sizes, level_one,
                vertices_count_up_to_level_one)
        level_two = PlanarSeparator._find_level_two(bfs_level_sizes, level_one,
                vertices_count_behind_level_one)

        cost_between_levels = bfs_level_costs[level_zero + 1:level_two].sum()

        if cost_between_levels < 2/3:
            return PlanarSeparator._mark_separation_if_cost_between_levels_is_less_than_two_thirds(
                    bfs_levels, bfs_level_costs, level_zero, level_two, cost_between_levels)

        # Step 6

        vertices_below_level_two_mask = ((bfs_levels != -1) & (bfs_levels < level_two))

        new_vertices_mapping1, _, new_graph = PlanarGraphConstructor.construct_subgraph(graph,
        ...
        new_bfs_levels = PlanarSeparator._map_levels(new_graph.size, new_vertices_mapping1,
                bfs_levels)
        bfs_tree_edges_mask = BFSTree.construct_bfs_tree_edges_mask(bfs_tree_root, new_graph)

        collapsed_vertex, new_vertices_mapping2, _, new_graph = \
                BFSTree.collapse_bfs_subtree(new_graph, new_bfs_levels, level_zero,
                bfs_tree_edges_mask)

        new_vertices_mapping = PlanarSeparator._get_new_vertices_mappings_composition(
                new_vertices_mapping1, new_vertices_mapping2)

        new_graph_separation = PlanarSeparator._mark_separation_of_graph_with_small_radius(
                new_graph, collapsed_vertex)

        new_graph_separation = \
                PlanarSeparator._swap_separation_parts_if_cost_of_first_part_is_smaller(
                new_graph.vertex_costs, new_graph_separation)
        ...
        separation[((bfs_levels == level_zero) | (bfs_levels == level_two))] = \
    ...
    def _mark_separation_if_cost_between_levels_is_less_than_two_thirds(bfs_levels,
        ...
        level_separation = PlanarSeparator._fill_undefined_with_second_part(level_separation)
    ...
    def _mark_separation_of_graph_with_small_radius(graph, center_vertex):
        ...
        bfs_tree_edges_mask = BFSTree.construct_bfs_tree_edges_mask(center_vertex, graph)

        parent_edge_indices, total_descendants_costs = \
                BFSTree.record_bfs_tree_parent_edge_indices_and_total_descendants_costs(graph,
                center_vertex, bfs_tree_edges_mask)

        new_edge_indices_mapping, graph = Triangulator.triangulate(graph)

        bfs_tree_edges_mask = PlanarSeparator._map_edges_mask(graph.edges_count,
                new_edge_indices_mapping, bfs_tree_edges_mask)

        parent_edge_indices = PlanarSeparator._get_new_parent_edge_indices(parent_edge_indices,
        ...
        cycle_vertices_mask, cycle_edges_mask, next_edge_indices_in_path_to_cycle = \
                TreeCycles.get_tree_cycle_masks_and_next_edge_indices_in_path_to_cycle(graph,
        ...
        tree_adjacency_costs_inside_cycle = \
                TreeCycles.iterate_tree_adjacency_costs_on_tree_cycle_side(graph,
        ...
        while cost_inside_cycle > 2/3:
            ...
            cost_inside_cycle, non_tree_edge_index, non_tree_edge_primary_vertex = \
                    TreeCycles.shrink_cycle(graph, bfs_tree_edges_mask, total_descendants_costs,
        ...
        for vertex in TreeCycles.iterate_vertices_on_cycle_side(graph, bfs_tree_edges_mask,
                non_tree_edge_primary_vertex, non_tree_edge_index, cycle_vertices_mask,
                cycle_edges_mask):
