...
def _permute_subgraph_vertices(graph, vertices_to_permute_mask, separation):
    ...
    #    print('{0} -> {1}'.format(v, list(graph.get_adjacent_vertices(v))))
    ...
    #print('in nd node', separator_mask.sum(), first_part_mask.sum(), second_part_mask.sum())
...
def permute_vertices(graph, top_level_separation):
    ...
    vertices_to_permute_mask = utils.repeat_bool(True, graph.size)
